package com.ricemarch.excelutil.common.excel;

import com.ricemarch.excelutil.common.excel.converter.DefaultConvertible;
import com.ricemarch.excelutil.common.excel.exceptions.Excel4jReadException;
import com.ricemarch.excelutil.common.excel.handler.ExcelHeader;
import com.ricemarch.excelutil.common.excel.handler.ExcelTemplate;
import com.ricemarch.excelutil.common.excel.utils.Utils;
...
public class ExcelUtils {
     ...
     * @param clazz      待绑定的类(绑定属性注解{@link .ExcelField})
    ...
    /**
     * 基于Excel模板与注解{@link .ExcelField}导出Excel
     *
     ...
     *
     * @param data          待导出数据
     * @param clazz         {@link .ExcelField}映射对象Class
     * @param isWriteHeader 是否写入表头
    ...
    public static void main(String[] args) {
        try {
            ...
            Workbook workBook = t.exportObjects2Excel(maps, header);
     ...
     * @author Crab2Died
     */
    public static Workbook exportObjects2Excel(List<?> data, List<String> header) throws Exception {

        return exportExcelNoModuleHandler(data, header, null, false);
    }
    ...
    public void exportObjects2Excel2(List<?> data, List<String> header, String sheetName, OutputStream os) throws Exception {

        exportExcelNoModuleHandler2(data, header, sheetName, true, os).write(os);
    }

    private static Workbook exportExcelNoModuleHandler(List<?> data, List<String> header,
                                                       String sheetName, boolean isXSSF) throws Exception {
    ...
    public Workbook exportExcelNoModuleHandler2(List<?> data, List<String> header,
                                                String sheetName, boolean isXSSF, OutputStream os) throws Exception {
        ...
        for (Object object : data) {
            ...
            for (int i = 0; i < header.size(); i++) {
                Object obj = map.get(header.get(i));
                row.createCell(i, Cell.CELL_TYPE_STRING).setCellValue(null == obj ? "" : obj.toString());
            }
            rowIndex++;
        }
        return workbook;
